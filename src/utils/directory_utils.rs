...
mod tests {
    use super::{
        fallback_sdkman_dir, infer_sdkman_dir, UserDirs, DEFAULT_SDKMAN_HOME, SDKMAN_DIR_ENV_VAR,
    };
    use rstest::rstest;
    use std::{
    ...
    fn infer_sdkman_dir_env_cases(#[case] env_set: bool) {
        ...
        if env_set {
            ...
            let _sdkman = ScopedEnvVar::set(SDKMAN_DIR_ENV_VAR, temp_dir.path());

            let got = infer_sdkman_dir().unwrap();
            assert_eq!(got, expected);
        } else {
            let _sdkman = ScopedEnvVar::remove(SDKMAN_DIR_ENV_VAR);
    ...
    fn infer_falls_back_when_env_is_not_unicode() {
        ...
        let _sdkman = ScopedEnvVar::set(SDKMAN_DIR_ENV_VAR, invalid);
